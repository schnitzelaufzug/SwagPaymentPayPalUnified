...
class ExpressCheckout implements SubscriberInterface
{
    ...
    {
        $productId = (int) $args->getSubject()->Request()->getParam('sArticle');
        if ($this->esdProductChecker->checkForEsdProducts([$productId]) === true) {
            return;
        }
    ...
    private function getProductIdsFromBasket(?array $content): array
    {
        if ($content === null) {
            return [];
        }
